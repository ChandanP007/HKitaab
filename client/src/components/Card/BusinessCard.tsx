  ...
  return (
    <>
      ...
      >
        <img
          src={src}
          className="rounded-full sm:rounded-none w-12 sm:w-72 h-12 sm:h-40 object-cover"
        />
        <div className="p-3 text-sm text-left flex items-center justify-between ">
          <section>
            <h1 className="text-sm sm:text-lg truncate w-52 font-bold">
              {title}
            </h1>
            <p className="font-extralight text-[12px] sm:text-[13px] sm:pt-2">
      ...
      <dialog
        open={showDialog}
        className="w-[95vw] min-h-[80vh] top-24 border-2 bg-gray-100 detail-modal transition-all z-20"
      >
        <button
          className="text-black p-5 text-2xl cursor-pointer float-right opacity-50 hover:opacity-100"
          onClick={(prev) => {
            setShowDialog(!prev);
          }}
        >
